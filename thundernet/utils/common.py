...
__all__ = ['is_channels_first', 'get_channel_axis', 'update_keras_shape', 'flatten', 'batchnorm', 'maxpool2d',
           'avgpool2d', 'conv2d', 'conv1x1', 'conv3x3', 'depthwise_conv5x5', 'conv_block', 'conv1x1_block',
...
def flatten(x,
    ...
    if not is_channels_first():
        def channels_last_flatten(z):
            z = K.permute_dimensions(z, pattern=(0, 3, 1, 2))  # 调整轴的次序
            z = K.reshape(z, shape=(-1, np.prod(K.int_shape(z)[1:])))
            update_keras_shape(z)
            print(z)
            return z

        return nn.Lambda(channels_last_flatten)(x) # 变成一个layer操作
    else:
        if reshape:
            x = nn.Reshape((-1,))(x)
        else:
...
def depthwise_conv5x5(x,
                      channels,
                      strides,
                      name="depthwise_conv5x5"):
    """
...
def channel_shuffle(x,
    ...
    if is_channels_first():
        ...
        batch, height, width, channels= x.shape[0], x.shape[1], x.shape[2], x.shape[3]
